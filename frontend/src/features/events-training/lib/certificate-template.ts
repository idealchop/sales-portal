/**
 * Client-side digital credential SVG for live preview.
 * Light River-inspired palette — keep aligned with backend template.
 */
...
export async function buildCertificateSvg(
  ...
  const recipient = escapeXml(
    truncate(input.recipientName || "Recipient name", 64),
  );
  const webinarName = escapeXml(
    truncate(input.courseName || input.title || "Webinar", 80),
  );
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="850" viewBox="0 0 1200 850" role="img" aria-label="Digital credential">
  ...
  <text x="600" y="202" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="13" letter-spacing="4" fill="#0f766e">${appLabel}</text>

  <text x="600" y="250" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="14" letter-spacing="3" fill="#0284c7">skill unlocked</text>
  <text x="600" y="298" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="42" font-weight="760" fill="#0f2f2e">Certificate of Completion</text>

  <g transform="translate(600, 324)">
    <rect x="-54" y="-4" width="108" height="8" rx="4" fill="url(#accent)"/>
  </g>

  <text x="600" y="368" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="13" letter-spacing="2.5" fill="#5b8a92">AWARDED TO</text>
  <text x="600" y="424" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="46" font-weight="700" fill="#123836">${recipient}</text>

  <text x="600" y="468" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="15" fill="#5b7c86">for attending</text>
  <text x="600" y="508" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="26" font-weight="700" fill="#0f766e">${webinarName}</text>
  <text x="600" y="542" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="16" fill="#3d6b7a">with ${speaker}</text>
  ...
  <text x="600" y="760" text-anchor="middle" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif" font-size="12" fill="#7a9aa2">river · ${certId}</text>
</svg>`;
}

export async function certificateSvgDataUrl(
  input: CertificatePreviewInput,
): Promise<string> {
