...
export type SubscriptionOfficialReceiptPdfInput = {
  businessName: string;
...
/** Renders a subscription Official Receipt PDF (A4). */
export function buildSubscriptionOfficialReceiptPdf(
  input: SubscriptionOfficialReceiptPdfInput,
): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        ...
        margin: 48,
        info: {
          Title: "Smart Refill Official Receipt",
      ...
      doc
        ...
        .fontSize(11)
        .font("Helvetica")
        .text("Official Receipt — Subscription", margin, doc.y, {
          width: contentWidth,
        });
      doc.moveDown(0.45);
      doc
      ...
      row("OR / reference", input.subscriptionId);
