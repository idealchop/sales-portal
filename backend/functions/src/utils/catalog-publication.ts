/**
 * Catalog draft/publish rules. Missing `catalogStatus` is treated as published.
 */
...
export const CATALOG_META_KEYS = new Set([
  "catalogStatus",
  "draft",
  "publishedAt",
  "publishedByUid",
...
export function commercialCatalogFields(
  data: Record<string, unknown>,
): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(data)) {
    if (CATALOG_META_KEYS.has(key)) continue;
    out[key] = value;
  }
  return out;
}
